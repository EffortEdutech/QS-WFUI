 ...
 * Uses OpenAI (via AiService) when configured; falls back to a
 ...
 *
 * Input (from qs.read_boq):
 *   ctx.inputs['boq'] — BOQDocument { items, currency, ... }
 *   OR ctx.inputs['boq_items'] — flat BOQItem[]
 *
 * Sprint 9 (S9-003)
 */
...
async function classifyWithAi(
  ...
  try {
    ...
    return null; // silently fall back to keyword classifier
  }
}
...
export async function realClassifyTrade(
  ...
  // Accept boq object (from qs.read_boq) OR flat boq_items array
  ...
  const useAi = ctx.config['use_ai'] !== false; // default true

  ctx.logger.info(`Classifying ${allItems.length} BOQ items (AI: ${useAi && aiService.isConfigured})`);

  // Separate section headers (classify as 'structural' boundary — handled separately)
  ...
  // ── Classify section headers (heuristic — mark as structural/civil boundary) ─
  for (const header of sectionHeaders) {
    classified.push({
      ...header,
      trade: 'others',
      confidence: 1.0,
      classified_by: 'section',
    });
  }
  ...
  if (useAi && aiService.isConfigured) {
    ...
    for (let i = 0; i < lineItems.length; i += BATCH_SIZE) {
      ...
      if (batchResult) {
        ...
        ctx.logger.warn(`AI batch ${Math.floor(i / BATCH_SIZE) + 1} failed — using keyword fallback for this batch`);
      }
    }
  ...
  // ── Merge AI results with keyword fallback ────────────────────────────────────
  for (const item of lineItems) {
    ...
    if (confidence < confidenceThreshold) {
      warnings.push(
        `Low confidence (${(confidence * 100).toFixed(0)}%) on "${item.description.slice(0, 60)}" → classified as ${trade}`,
      );
    }
