 ...
 *          WorkPackage / ClassifiedItem now imported from @lados/qs-pack.
 */
import {
...
import type { WorkPackage, ClassifiedItem } from '@lados/qs-pack';

// ── Local ILibraryService interface ──────────────────────────────────────────
// NestJS LibraryService satisfies this via duck typing.
  ...
  return new TableCell({
    ...
    children: [
      new Paragraph({
        ...
        children: [
          new TextRun({ text, bold: opts.bold, size: opts.small ? 16 : 18 }),
        ],
      }),
    ],
  });
...
function buildRfqDocument(
  ...
  const titleBlock = [
    new Paragraph({
      ...
      children: [new TextRun({ text: 'REQUEST FOR QUOTATION', bold: true, size: 48, color: BRAND_BLUE })],
    }),
    new Paragraph({
      alignment: AlignmentType.CENTER,
    ...
    new Paragraph({
      ...
      children: [new TextRun({ text: `Trade Package: ${label}`, size: 22, color: '555555' })],
    }),
  ];
  ...
  const infoTable = new Table({
    ...
    rows: [
      new TableRow({ children: [cell('RFQ Reference', { bold: true, shade: true }), cell(rfqRef), cell('Date Issued', { bold: true, shade: true }), cell(issuedDate)] }),
      new TableRow({ children: [cell('Project', { bold: true, shade: true }), cell(projectName), cell('Response Due', { bold: true, shade: true }), cell(closingDate)] }),
      new TableRow({ children: [cell('Trade Package', { bold: true, shade: true }), cell(label), cell('Currency', { bold: true, shade: true }), cell(currency)] }),
    ],
  });
  ...
  const termsSection = [
    ...
    new Paragraph({
      heading: HeadingLevel.HEADING_2,
      spacing: { before: 300, after: 100 },
      children: [new TextRun({ text: `${sectionNum}.  TERMS AND CONDITIONS`, bold: true, size: 24, color: BRAND_BLUE })],
    }),
    ...[
